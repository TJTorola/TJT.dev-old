...
const generateRandomSvgProps = (x, y) => ({
  maze: {
    ...
    rows: emptyArr(y).map(() => emptyArr(x).map(randColor)),
    walls: {
      vertical: emptyArr(x - 1).map(() => emptyArr(y).map(() => randNum(2))),
      horizontal: emptyArr(y - 1).map(() => emptyArr(x).map(() => randNum(2)))
    }
  }
});
...
export const testRandomGrid = (type, x, y) => {
  ...
  const generateRandomProps =
    type === "CANVAS" ? generateRandomCanvasProps : generateRandomSvgProps;

  const Grid = type === "CANVAS" ? CanvasGrid : SvgGrid;
